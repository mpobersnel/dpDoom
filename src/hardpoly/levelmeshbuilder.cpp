...
void LevelMeshBuilder::Render(HardpolyRenderer *renderer, const std::vector<subsector_t*> &seenSubsectors, const std::vector<sector_t *> &seenSectors)
{
	...
	for (size_t i = 0; i < mNextBatch; i++)
	{
		auto batch = mCurrentFrameBatches[i].get();
		if (batch->Producer != &mMainThread && !batch->DrawRuns.empty())
		{
			if (!batch->Vertices)
...
void LevelMeshBuilder::GetVertices(LevelMeshThread *thread, int numVertices, int numIndices)
{
	...
	if (thread->mVertices == nullptr)
	{
		...
		thread->mCurrentBatch = mCurrentFrameBatches[mNextBatch++].get();
		thread->mCurrentBatch->DrawRuns.clear();
		thread->mCurrentBatch->Producer = thread;
