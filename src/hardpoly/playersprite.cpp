...
void HardpolyRenderPlayerSprites::Render()
{
...
void HardpolyRenderPlayerSprites::RenderRemainingSprites()
{
	for (const HardpolyHWAccelPlayerSprite &sprite : AcceleratedSprites)
	{
...
void HardpolyRenderPlayerSprites::RenderSprite(DPSprite *pspr, AActor *owner, float bobx, float boby, double wx, double wy, double ticfrac, int spriteshade, FDynamicColormap *basecolormap, bool foggy)
{
	double 				tx;
	int 				x1;
	int 				x2;
	...
	const auto &viewpoint = r_viewpoint;
	const auto &viewwindow = r_viewwindow;
	DCanvas *renderTarget = screen;// PolyRenderer::Instance()->RenderTarget;
	...
	HardpolyNoAccelPlayerSprite vis;
	...
	if (!noaccel && renderTarget == screen && (DFrameBuffer *)screen->Accel2D)
	{
		...
		{
			HardpolyHWAccelPlayerSprite accelSprite;
...
fixed_t HardpolyRenderPlayerSprites::LightLevelToShade(int lightlevel, bool foggy)
{
...
void HardpolyNoAccelPlayerSprite::Render()
{
#if 0
...
void HardpolyColormapLight::SetColormap(double visibility, int shade, FDynamicColormap *basecolormap, bool fullbright, bool invertColormap, bool fadeToBlack)
{
	if (fadeToBlack)
	{
