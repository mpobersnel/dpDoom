...
class HardpolySkySetup
{
public:
	void Update();

	bool operator==(const HardpolySkySetup &that) const { return memcmp(this, &that, sizeof(HardpolySkySetup)) == 0; }
	bool operator!=(const HardpolySkySetup &that) const { return memcmp(this, &that, sizeof(HardpolySkySetup)) != 0; }
...
class HardpolySkyDome
{
public:
	HardpolySkyDome();
	void Render(HardpolyRenderer *renderer);
...
private:
	...
	GPUVertexBufferPtr mVertexBuffer;
	...
	Vertex SetVertexXYZ(float xx, float yy, float zz, float uu = 0, float vv = 0);
};
