...
{
public:
	HardpolyPlane() : A(0.0f), B(0.0f), C(0.0f), D(1.0f) { }
	HardpolyPlane(float a, float b, float c, float d) : A(a), B(b), C(c), D(d) { }

	float A, B, C, D;
};

class HardpolyBSPCull
{
public:
	void ClearSolidSegments();
	void CullScene(const HardpolyPlane &portalClipPlane);
